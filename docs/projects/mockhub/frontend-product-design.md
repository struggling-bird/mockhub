...
- **表单字段**
  - 登录模式：邮箱、密码。
  - 注册模式：用户名、公司名、邮箱、密码。
  - 邀请注册模式：展示项目邀请上下文，邮箱和公司名称由邀请默认填充并锁定，只允许填写用户名和密码。
  - 所有字段标签及占位符均提供中英双语。
- **语言切换**
...
- **指标卡片**
  ...
  - Team：团队成员数。
  - Requests/hr：每小时请求量。
  - 数据由后端 `/api/dashboard/summary` 提供；Team 指标按当前可见项目的 Owner 与 Active 成员去重统计。
...
- **页面分区**
  - Team 页面采用「成员管理 / 邀请管理 / 角色权限」Tab 分区。
  - 成员管理默认展示正式成员，用于处理当前团队成员、角色调整、状态调整和移除。
  - 邀请管理独立承载邮箱邀请、自动复制邀请消息、待接受邀请复制和撤销。
  - 角色权限独立承载角色列表、角色详情和分组权限编辑，避免横向矩阵在自定义角色增多后难以维护。
- **成员信息**
  - 数据来自 `/api/projects/:projectId/members`。
  - 显示成员昵称首字母头像、姓名、邮箱、项目角色、状态（Active/Inactive）。
  - Owner 由项目创建者生成虚拟成员行展示，不写入普通成员表。
- **操作能力**
  - Owner/Admin 可输入任意合法邮箱邀请成员。
  - 邀请成功后生成邀请消息和链接，前端自动复制。
  - 待接受邀请独立展示，支持复制邀请和撤销邀请。
  - 未注册用户可通过邀请链接进入注册页，注册完成后自动加入项目。
  - 邀请注册页默认填充并锁定邀请邮箱和公司名称，避免被邀请账号修改注册归属。
  - Owner/Admin 可调整普通成员角色和状态，可移除普通成员。
  - Editor/Viewer 只能查看成员列表。
  - 筛选成员（Filter members…）。
  - 查看总人数统计。
- **角色管理**
  - Team 页面采用左侧角色列表、右侧当前角色详情和分组权限编辑交互。
  - Owner 固定全权限并锁定；Owner 可新增自定义角色、复制角色、编辑角色名称/描述/权限，并删除未被成员或待接受邀请引用的自定义角色。
  - 成员管理和邀请管理中的角色选项来自项目角色定义，不再写死 Admin、Editor、Viewer。
- **权限边界**
  - Owner：完整管理项目、成员、接口和公共资产。
  - Admin：可管理成员、编辑项目、维护接口和公共资产。
  - Editor：可维护接口和公共资产。
  - Viewer：只读项目、接口、公共资产和团队成员。
