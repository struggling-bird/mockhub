...
- Proxy 目标地址候选会合并项目公共资产中的 Base URL，公共资产不再只停留在单独维护页面。
...
- Mock：
  ...
  - 静态 Mock 与脚本 Mock 编辑区提供公共资产引用入口，可复制或插入错误码、枚举、JSON 配置和文本常量。
...
- **产品价值**
  - 为脚本 Mock 提供可复用错误码、枚举值和共享配置。
  - 为真实代理提供 Base URL 候选，降低代理目标地址重复维护成本。
  - 为接口定义和测试场景提供统一的项目级枚举、错误码和共享 Schema 参考。
  - 将团队约定从单个接口、脚本和代理规则中沉淀出来，形成可复制、可搜索、可维护的公共上下文。
- **资产列表**
  - 资产数据来自项目级后端接口 `/api/projects/:projectId/assets`。
  - 每条资产包含名称、值、类型和类别，当前支持 Base URL、Error Codes、Enum Set、JSON Config、Text Constant。
  - 支持新增、编辑、删除、搜索和复制资产值。
- **页面布局**
  - 公共资产页采用「正式资产 / 建议资产」Tab 布局，默认展示正式资产。
  - 搜索和新增操作仅作用于正式资产，建议资产独立展示，避免建议数量过多影响正式资产维护。
  - 正式资产和建议资产均采用紧凑行式列表，不使用大卡片墙，便于高密度扫描、复制、编辑和确认入库。
- **新增与编辑**
  - 新增资产时先选择资产类型，再进入对应类型的编辑交互。
  - Base URL 使用 URL 输入框。
  - Error Codes 使用 code/message 多行结构，支持增删错误码项。
  - Enum Set 使用枚举项列表，支持增删枚举值。
  - JSON Config 使用 JSON 编辑区，支持保存前校验和手动格式化。
  - Text Constant 使用多行文本编辑区。
- **空态引导**
  - 强调「集中管理常量」的价值，鼓励用户维护错误码、基础 URL、共享 Schema 等。
- **建议资产**
  - 后端基于项目代理地址、接口代理地址、请求日志、接口参数、响应结构和静态 Mock 自动识别候选公共资产。
  - 当前建议类型包括 Base URL、HTTP 错误状态码集合、业务响应码、字段枚举集合和通用响应结构模板。
  - 请求参数与响应结构中的 `status`、`state`、`type`、`role`、`channel`、`mode`、`code` 等字段会作为枚举或业务码候选来源。
  - 静态 Mock JSON 中的 `{ code, message, data }`、`{ success, data, error }`、`{ status, message, data }` 会作为通用响应结构候选来源。
  - 自动识别结果先进入建议资产 Tab，展示来源、出现次数和置信度，用户可打开详情侧栏查看完整取值、识别来源、置信度等信息。
  - 建议资产支持确认入库和忽略；忽略状态按项目保存在浏览器本地，避免用户不接受的建议反复干扰审核。
  - 用户确认后才写入正式公共资产，避免误入库和敏感信息泄漏。
