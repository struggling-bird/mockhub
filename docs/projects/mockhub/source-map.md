...
| `src/app.module.ts` | 聚合 Prisma、Redis、Auth、Projects、Upload、Apis、Gateway、Dashboard、Assets 模块 |
...
| `src/assets/` | 项目公共资产 CRUD 与建议资产接口，按项目 owner 隔离基础 URL、枚举、错误码等共享配置 |
| `src/dashboard/` | Dashboard 统计接口，聚合项目接口数、代理配置数、最近一小时请求量和最近网关活动 |
...
| `prisma/schema.prisma` | User、Project、ProjectApi、RequestLog、PublicAsset 数据模型 |
