...
export class CreateProxyGroupDto {
  @ApiProperty({ description: '代理分组名称', example: 'User APIs' })
  name: string;

  @ApiProperty({ description: '路径正则表达式', example: '^/api/users' })
  regex: string;

  @ApiProperty({ description: '代理模式：Mock | Proxy | Hybrid', example: 'Hybrid' })
  mode: string;
